...
def KNN(x):
  # https://github.com/getkeops/keops/tree/3efd428b55c724b12f23982c06de00bc4d02d903
    ...
    # adj = torch.sparse_coo_tensor(ei.cpu(), [1]*len(ei[0]), (x.shape[0], x.shape[0])).cuda()
    adj = sparse.coo_matrix(([1]*len(ei[0]), (ei.cpu().detach().numpy()[0,:], ei.cpu().detach().numpy()[1,:])))
    return adj
    ...
    def train(self,
        ...
        if train_mode == "inductive":
            ...
            for epoch in epoch_bar:
                ...
                if rewire and epoch % 5 == 0 and epoch != 0:
                    adj_train = KNN(features_train)
                    adj_train = utils.adj_preprocess(adj_train,
                                             adj_norm_func=model.adj_norm_func,
                                             model_type=model.model_type,
                                             device=self.device)
