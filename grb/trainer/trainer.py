    ...
    def train_step(self, model, features, adj, labels, pred_mask=None, labels_mask=None):
        ...
        if train_loss is None:
            ...
            if self.loss == F.binary_cross_entropy or self.loss == F.binary_cross_entropy_with_logits:
                labels = labels.float()
            if self.smooth_reg is None:
                train_loss = self.loss(pred, labels)
            else:
                print(len(model.att))
                mean_att = torch.cat(model.att, 0).detach()
                target_att = torch.ones(mean_att.size()).to(self.device)
                smooth_loss = F.mse_loss(mean_att, target_att)
                train_loss = self.loss(pred, labels) + self.smooth_reg*smooth_loss
