...
class RewireAttODEblock(ODEblock):
  ...
  def forward(self, x):
    ...
    if self.adjoint:
        ...
        #state_dt = integrator(func, x, t, method='dopri5')
        state_dt = integrator(func, x, t, method='implicit_adams', options=dict(step_size=5))
...
class GCNConv(nn.Module):
    ...
    def __init__(self,
        ...
        if self.training:
            adj_tensor = utils.adj_preprocess(dataset.adj,
                                       adj_norm_func=adj_norm_func,
            ...
            adj_tensor = utils.adj_preprocess(dataset.adj,
                                       ...
                                       device=device)
        self.pdeblk = RewireAttODEblock(ODEFuncTransformerAtt, out_features, out_features, adj_tensor, torch.tensor([0, 1]), self.training)
