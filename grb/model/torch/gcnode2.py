...
class ODEFuncTransformerAtt(ODEFunc):
  ...
  def __init__(self, in_features, out_features, adj):
    ...
    self.edge_index = adj._indices().to(device)
    self.edge_attr = adj._values().to(device)
    self.edge_index, self.edge_weight = add_remaining_self_loops(self.edge_index, self.edge_attr, fill_value=1)
...
class GCNConv(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        block = RewireAttODEblock(ODEFuncTransformerAtt, self.out_features, self.out_features, adj, self.time_tensor)
